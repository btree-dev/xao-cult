...
7. **The inbox topic replay mechanism, on its own, does not actually give a fresh device or a rotated session access to prior content.** `createSessionKeypair()` (`session.ts`) mints a random keypair per unlock, not one deterministically derivable from the wallet — so an inbox notice ECIES-encrypted to "my current session pubkey" is only ever decryptable by the exact session that generated it. A different device, or the same device after its 30-day session rotates, derives a *different* ECDH secret and cannot read history encrypted under the old one. This is a pre-existing property of the whole session-key system (DMs already have it), not something this spec introduces — but §5's mint-pairing mechanism inherits it directly: it is real and correct within one continuous session/device, but does **not** deliver on a literal "any device, anytime" guarantee. See §11.
...
**Mint pairing.** At mint, in addition to the existing in-thread `SYSTEM` "minted" message, an event `ThreadNotice` carrying `contractAddress` is (re-)published to both party1's and party2's inbox topics — same notice, same validation, just with `contractAddress` now populated. This is what lets either party's *current session* replay its own inbox and answer "given this on-chain contract address, which draftId/thread/key does it belong to?" without needing local history from the negotiation (§8) — within that session's lifetime. Per §2 fact 7, it does not extend to a genuinely fresh device or a session past its 30-day rotation, since the notice is encrypted to that specific session's key.
...
- **Mint continuity does not survive a genuinely fresh device or a session-key rotation, not just Waku's store retention window.** (Added post-implementation, during final review.) Session keypairs (`createSessionKeypair()` in `session.ts`) are random per unlock, not derived deterministically from the wallet — this is a pre-existing property of the whole session-key system, not introduced here, but §5's mint-pairing notice is encrypted to one specific session's public key, so only that same session/device can ever decrypt it. A device that was live for the original mint notice, within its current 30-day session, gets real continuity. A fresh device — or the same device after its session has rotated — cannot decrypt old notices at all and always falls back to the legacy `threadIdForShow` path, regardless of store retention. Closing this for real requires deterministic, wallet-signature-derived session keys (so the same wallet always re-derives the same keypair) — a change to `session.ts` affecting DMs too, scoped as separate future work, not part of this spec.
- Mint-continuity resolution is *additionally* bounded by Waku's store retention window within a single valid session — a session logging in for the first time after that window has passed for a given mint notice falls back to the legacy path rather than failing.
- **Follow-up (non-blocking, pre-existing, found during final review):** `useXaoEvent.ts`'s `notifyThread` caches a resolved peer session cert in `peerCertRef` without validating it against the `peer` the hook is currently negotiating for, or clearing it on a `peer`/`draftId` change mid-flight. If a user switches negotiations while a peer-cert lookup for the previous one is still in flight, a stale cert could theoretically be used to encrypt a notice intended for the new peer, which the new peer's device would fail to decrypt (silent, no data exposure to them) while leaking that notice's metadata (draftId, contractAddress, timing) onto the *old* peer's publicly-derivable inbox topic — readable only by that old, unrelated peer, not a third party. Confirmed pre-existing (not introduced by any fix round) and does not affect `useResolveEventThread`'s draft-matching guarantees. Fix is small: pin `const targetPeer = peer` at the start of the lookup and validate `cert.walletAddress` against it before using/caching.

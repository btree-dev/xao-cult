...
const Dashboard: NextPage = () => {
  ...
  const [filterLocation, setFilterLocation] = useState<LocationFilterData>({
    name: "",
    coordinates: null,
  });
  ...
  } = useAllContractsWithSummaries(chain?.id || DEFAULT_CHAIN);
  ...
  const usdcAddress =
    usdcChainId === base.id ? USDC_ADDRESS_MAINNET : USDC_ADDRESS_TESTNET;
  ...
  const formattedUSDC = usdcAmount.toLocaleString("en-US", {
  ...
  const formattedUNI = onBaseMainnet
    ? uniAmount.toLocaleString("en-US", {
        minimumFractionDigits: 2,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const filtered = await applyContractFilters(
        contracts,
        dateFilters,
        filterLocation,
        currentUserProfile,
    ...
    <Layout>
      ...
      <div className={styles.dashboardContainer}>
        ...
        <div className={styles.walletCardContainer}>
          ...
          >
            <div className={styles.walletCardHeader}>
              <span className={styles.walletUsername}>
                @{currentUserProfile?.username || "yevhenii_d"}
              </span>
            </div>
            <div
            ...
            >
              ...
              <div className={styles.walletCurrencyRight}>
                <span className={styles.walletCurrencyValue}>
                  {formattedUSDC}
                </span>
                <span className={styles.walletCurrencyUsd}>
                  ({formattedUSDC} usd)
                </span>
              </div>
            </div>
        ...
        <div className={styles.feedContainer}>
            ...
            >
              ...
              <div className={styles.feedContent}>
                ...
                <div className={styles.feedContentOverlayTop}>
                  <h2 className={styles.feedEventTitle}>
                    {contract.eventName || "Untitled Contract"}
                  </h2>
                    ...
                    handleShare(
                      {
                        ...
                        title: contract.eventName || "Contract",
                      },
                      e,
                    )
                  }
                ...
                >
                  ...
                  >
                    <path
                      ...
                      fill={
                        likedEvents.has(contract.contractAddress)
                          ? "#DC143C"
                          : "none"
                      }
                      stroke={
                  ...
                  <span className={styles.actionCounter}>
                    {likedEvents.has(contract.contractAddress) ? 1 : 0}
                  </span>
                </div>
                <div
                  className={styles.actionButton}
          ...
          {/* Empty state — reached only after loading (the loading UI returns
              earlier), so an empty list here means the user genuinely has no
              events/contracts yet. */}
